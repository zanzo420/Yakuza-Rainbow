...
uint64_t kdmapper::MapDriver(HANDLE iqvw64e_device_handle, int driver) // pass in our argument lol
{
	//std::vector<uint8_t> raw_image = { 0 };
	std::vector<uint8_t> raw_dr;

	if (driver == 1) // basically our byte array lol
	{
		std::vector<uint8_t> dr1(intel_driver_resource::driverM, intel_driver_resource::driverM + sizeof intel_driver_resource::driverM / sizeof intel_driver_resource::driverM[0]);
		raw_dr = dr1;
	}
	...
	const PIMAGE_NT_HEADERS64 nt_headers = portable_executable::GetNtHeaders(raw_dr.data());

	if (!nt_headers)
	{
		//"[-] Invalid or non-x64 PE image"
		std::cout << "[-] Corrupted Image if problem persist contact admin" << std::endl;
		Sleep(5000);
		std::exit(0);
	}
	...
	do
	{
		if (!kernel_image_base)
		{
			...
			std::cout << "[-] Not Enough Space if problem persist contact Admin" << std::endl;
			Sleep(5000);
			std::exit(0);
			break;
		}
		...
		memcpy(local_image_base, raw_dr.data(), nt_headers->OptionalHeader.SizeOfHeaders);
		...
		for (auto i = 0; i < nt_headers->FileHeader.NumberOfSections; ++i)
		{
				...
				section_headers[i].PointerToRawData != 0)
			{
				...
				memcpy(local_section, reinterpret_cast<void*>(reinterpret_cast<uint64_t>(raw_dr.data()) + section_headers[i].PointerToRawData), section_headers[i].SizeOfRawData);
			}
		}
		...
		if (relocs.empty() && (nt_headers->FileHeader.Characteristics & IMAGE_FILE_RELOCS_STRIPPED) != 0)
		{
			...
			std::cout << "[-] Can't Find Image if problem persist contact Admin";
			Sleep(5000);
			std::exit(0);
			break;
		}
		...
		if (!ResolveImports(iqvw64e_device_handle, portable_executable::GetImports(local_image_base)))
		{
			...
			std::cout << "[-] Can't Find Imports if problem persist contact Admin" << std::endl;
			Sleep(5000);
			std::exit(0);
			break;
		}
		...
		if (!intel_driver::WriteMemory(iqvw64e_device_handle, kernel_image_base, local_image_base, image_size))
		{
			...
			std::cout << "[-] Failed to Write if problem persist contact Admin" << std::endl;
			Sleep(5000);
			std::exit(0);
			break;
		}
		...
		if (!intel_driver::CallKernelFunction(iqvw64e_device_handle, &status, address_of_entry_point))
		{
			...
			std::cout << "[-] Failed to Call if problem persist contact Admin" << std::endl;
			Sleep(5000);
			std::exit(0);
			break;
		}
...
bool kdmapper::ResolveImports(HANDLE iqvw64e_device_handle, const portable_executable::vec_imports& imports)
{
	for (const auto& current_import : imports)
	{
		if (!utils::GetKernelModuleAddress(current_import.module_name))
		{
			...
			std::cout << "[-] Dependency wasn't found if problem persist contact Admin" << std::endl;
			Sleep(5000);
			std::exit(0);
			return false;
		}
		...
		for (auto& current_function_data : current_import.function_datas)
		{
			...
			if (!function_address)
			{
				...
				std::cout << "[-] Failed to resolve import if problem persist contact Admin" << std::endl;
				Sleep(5000);
				std::exit(0);
				return false;
			}
